...
<body>
	...
	<section class="d-flex justify-content-center">
		<div class="form-container px-5 py-4">
			...
			<form action="<%=baseURL%>/khach-hang" method="post">
				<input type="hidden" name="action" value="update-profile"> <input
					type="hidden" name="maKH" value="<%=khachHang.getMaKH()%>">

				<div class="mb-3">
					<label for="customerName" class="form-label"><i class="bi bi-person me-1"></i>Tên
						khách hàng</label> <input type="text" class="form-control input-disabled" id="customerName"
						name="customerName" value="<%=khachHang.getTenKH()%>"
						placeholder="Nhập tên khách hàng" required disabled>
				</div>

				<div class="mb-3">
					<label class="form-label"><i class="bi bi-gender-ambiguous me-1"></i>Giới
						tính</label>
					<div>
						<div class="form-check form-check-inline">
							<input class="form-check-input input-disabled" type="radio" name="gender" id="male"
								value="Nam" <%="Nam".equals(khachHang.getGioiTinh()) ? "checked" : ""%> disabled>
							<label class="form-check-label" for="male">Nam</label>
						</div>
						<div class="form-check form-check-inline">
							<input class="form-check-input input-disabled" type="radio" name="gender"
								id="female" value="Nữ" <%="Nữ".equals(khachHang.getGioiTinh()) ? "checked" : ""%>
								disabled> <label class="form-check-label" for="female">Nữ</label>
						</div>
					</div>
				</div>

				<div class="mb-3">
					<label for="dob" class="form-label"><i class="bi bi-calendar me-1"></i>Ngày
						sinh</label> <input type="date" class="form-control input-disabled" id="dob" name="dob"
						value="<%=khachHang.getNgaySinh() != null ? khachHang.getNgaySinh() : ""%>"
						placeholder="Chọn ngày sinh" required disabled>
				</div>

				<div class="mb-3">
					<label for="phoneNumber" class="form-label"><i class="bi bi-telephone me-1"></i>Số
						điện thoại</label> <input type="tel" class="form-control input-disabled" id="phoneNumber"
						name="phoneNumber" value="<%=khachHang.getSoDienThoai()%>"
